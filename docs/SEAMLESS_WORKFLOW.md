...
5. 若有待標註資料，LabelImg 會自動開啟並載入正確類別。檔名前綴會直接標示這張要做什麼（`1-補框`、`2-刪錯框`、`3-改類別`、`4-調整框`、`5-待確認`），依檔名排序即可把同類工作集中處理。漏檢案件依序操作「拖曳框選 → 選擇類別 → `Ctrl+S` → 關閉工具」，系統會自動驗證並回流 `raw`；未完成時才顯示「開始標註」供繼續處理。誤檢與錯類案件即使已有系統產生的草稿框，也必須實際修正並儲存；直接關閉 LabelImg 不會把未確認草稿送入訓練。若其中一張標註不合法，這一輪不會加入任何資料，其餘影像維持原狀，修正後再試一次即可。
...
- 組長補訓模式要求每類至少 5 個有效 instance，其中 train 至少 3 個、test 至少
  5 個；train/val/test 圖片數至少為 3/5/10。切分另外要求 val 至少 5 組、test
  至少 10 組互相獨立的原圖，且這些組別必須來自本次以外的歷史資料——因此本次
  送出的照片之外，還需要累積至少 15 張先前送出的可訓練影像才會開始補訓。產品
  設定只能提高，不會降低這些安全下限。
- 推理端在送出前就會用同一組下限估算並顯示「還需要約 N 張」，資料不足時仍可送
  出累積，只是這一批不會啟動訓練。
