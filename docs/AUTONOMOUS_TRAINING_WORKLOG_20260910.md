...
## 17. 更正 §15，以及 pin 升不動的真正原因（2026-09-15）

### 先更正 §15 的一句話

§15 說「`mypy src/picture_tool` 是 CI blocking gate，所以升級不能就這樣過」。
**那 12 個錯不會出現在 CI**，這句話是錯的。

CI 的 `quality` job（`ci.yml:20-44`）**只裝 `requirements-quality.txt`**
（mypy／ruff／types-PyYAML），**沒有 ultralytics**。配上
`ignore_missing_imports = true`，ultralytics 在 CI 眼中就是 `Any`，
union-attr 根本不會被觸發。

實測驗證（不是推論）：用 CI 完全相同的 pin（mypy 1.19.1、ruff 0.14.10、無
ultralytics）建一個乾淨 venv，對**修正前**的 worktree 跑 `mypy src/picture_tool`
—— `Success: no issues found in 147 source files`。

**推論**：CI 的型別閘門**抓不到任何 ultralytics 相關的型別錯誤**，
8.3 或 8.4 都一樣。這是閘門本身的缺口，不是這次升級的問題。
那 12 個錯是真的（本機、乾淨快取、兩次確認），只是**只有裝了 ultralytics 的地方才看得到**。

型別修正（`01ca276`）仍然值得留著：它在 8.3.156 與 8.4.152 下都 clean，
不依賴 pin 動不動。

### pin 沒有升，因為 lockfile 早就與它的輸入脫節

想照規矩走（改 `.in` → `pip-compile` 重新產生 lock），結果重新編譯
**刪掉 677 行**：`dvc[s3]`、`fastapi`、`uvicorn[standard]`、`typer`、`celery`、
`boto3` 等約 90 個套件全部消失。

**原因不是升級，是既有的脫節**：`requirements.in` 把 fastapi／uvicorn／
python-multipart／dvc 放在「===== OPTIONAL DEPENDENCIES =====」底下**註解掉**，
但 committed 的 `requirements.txt` 裡**有**它們。`requirements-dev.in` 同樣沒提到
typer／fastapi／uvicorn／dvc，而 `requirements-dev.txt` 裡也有。
兩份 lock 都是在那些行還沒被註解掉的年代產生的，之後沒有人重新編譯過。

刪掉會實際弄壞 CI，而且是可預測的：
- 少了 `typer` → `test_autotrain_cli.py` 在 CI 也變成 skip（本機本來就 skip，
  CI 是**唯一**真的跑它的地方，見 §4）。
- 少了 `fastapi`/`uvicorn` → `test_coverage_serve_edges.py` 7 個測試失敗。

**為什麼不手改 lock**：那兩個檔案的 header 寫著 autogenerated by pip-compile。
手改會讓 header 變成謊話，而且下一個人執行 pip-compile 時同樣的 677 行刪除會再發生一次
——等於把陷阱重新上膛，還埋得更深。

### 要升 pin，得先決定一件事（這是產品決定，不是技術決定）

`fastapi`／`uvicorn`／`dvc`／`mlflow` 到底**算不算**這個專案的依賴？

- **算** → 把 `requirements.in` 裡那幾行取消註解，重新編譯，lock 會回到現狀 + ultralytics 升版。
- **不算** → 接受那 677 行刪除，但要同時處理 CI 會壞掉的兩處測試。
...
## 19. 冷啟動：LLM 標註 → 訓練（2026-09-15）

使用者的最終目標是**冷啟動**：新站別給幾張圖 + 一句描述 → 自動標註 → 訓練。
§1–§18 對的是「已有 champion 之後的持續改進」，那是暖啟動，兩回事。

### 現成的開放詞彙模型不能用（實測掉了）

| 模型 | 提示 | 結果 |
| --- | --- | --- |
| YOLO-World | 五色 wire | **0 框** |
| YOLO-World | `wire` / `cable` | **0 框** |
| YOLOE | 五色 wire | 15 框，top conf **0.093**，分布全錯 |

原因是域差距：3072×2048 微距特寫、畫面是焊點上的線頭，不是網路影像裡的「電線」。
**ultralytics 為此自動往系統環境裝了 `clip`/`ftfy`/`open_clip_torch`/`regex`**，不是人裝的。

### VLM 可以，但能力極度不對稱

實測（今天 6 張已確認影像，以及人工標註 46 張）：

| 面向 | 表現 |
| --- | --- |
| 類別、數量 | **完美**。36/36 框、6/6 正確多重集 |
| **cx（水平位置）** | **±0.005**，準到不可思議 |
| 寬度 | 對（0.045 vs 人工 0.041–0.047） |
| 高度 | **約真值的一半** |
| **cy（垂直位置）** | **壓平成常數**，不跟板子的傾斜走 |

所以架構是：**LLM 負責分類與水平定位，垂直範圍必須另外校正。**

### 兩個「描述」的差別，踩了兩次才分清楚

- 提示裡講**位置**（「y 中心約 0.62」）→ 框跟著提示走不看圖，物件在別處的畫面崩到 IoU 0.03。
- 提示裡**完全不講物件是什麼** → 它框整條線（延伸出畫面），一個都對不上。

**規格要給，答案不能給。** `VisionPrompt.object_description` 就是這條線。

### 只有真跑才會撞到的三件事

1. **`judge()` 職責錯位**：它硬要 verdict 欄位，問座標會被當協定錯誤重試兩次。
   拆成 `ask()`（傳輸＋解析）與 `judge()`（在其上驗證 verdict）。
2. **模型用自己訓練的單位回答**：回來是 **0–1000 網格**，而且**有一筆在同一個框裡混用**
   （x 像素、y 正規化）。靠數值大小無法分辨：`233` 在 0–1000 是 0.233，在 640 幀是 0.364。
   改成**用模型原生單位發問、只認那一種慣例**，超出畫面的丟棄不裁切。
3. **推理模型會吃光 `max_tokens`**：3000 仍有整張圖被截斷，預設提到 6000。

### 端到端跑通了

今天 6 張 → LLM 標註 → 校正 → 增強成 113 張 → lint → 切分 → 訓練。
`trained_this_run: True`，class schema hash 不變。

**mAP50 0.719 / mAP50-95 0.117** —— 寬鬆門檻好、嚴格門檻差，正是
「類別對、框不準」的指紋，獨立印證了標註品質的量測。
...
| --- | --- | --- |
| 偵測器的框 | 「像不像現行偵測器」 | 對不對 |
| **驗收集 250 張** | **OK/NG 判定對不對**（人工確認） | 框對不對 |
| **handoff raw 46 張** | **框對不對**（人工標註） | —— |

**先前反覆說的「沒有人工真值」是錯的。** 兩份都在，只是回答不同問題。

### 驗收集揭露的事：headroom 只有 3 筆

`station_data/yolo11_inference/acceptance/Cable1/A/`，250 筆全部 `review_status: confirmed`。

部署模型 vs 人工真值：**accuracy 0.988、precision 0.963、recall 1.000、漏檢 0**。

三筆誤報全部**不是類別判錯**：2 筆 `SEQUENCE_MISMATCH`（顏色數量全對）、
1 筆多偵測一個框。

**推論**：`VisionLLMEvidence`（§18 建的）對著這份唯一真值的可測量效益是 **0**。
它正確、有測試、只能增加 reason 不會造成傷害，但**不要當成已驗證的改善**。

也因此，autotrain 對 Cable1/A 的價值應定位成**維持**而非提升；
冷啟動的價值在**下一個站別**。

### 一個自己提出又自己推翻的假設

曾懷疑順序檢查拿信心排序去比對空間預期。**查過程式碼，錯了**——
`core/services/detection_sequence.py` 的 `left_right_sequence` 確實依 `_center_x` 排序，
註解還特別說明為何需要單一真相來源。那 2 筆為什麼順序不符，現存資料回答不了，
要重跑推論（GUI 的「全部重新推論」）。

### 停在哪 —— 下次接手從這裡開始

對人工真值的完整量測**沒有跑完**，因為發現匹配邏輯本身有錯：

`match_by_centre` 用**二維歐氏距離**，而 cy 是三個維度裡唯一不可信的。
結果是 cx 完全正確（±0.005）、類別 6/6 全對的畫面被判成 **0/6 沒對上**。
**拿不可信的維度去否決可信的維度**，這是 bug 不是模型表現。

兩件要做的事：
1. **匹配以 cx 為主**（或給 cy 較低權重）。
2. **`BoxCalibration` 要加傾斜項** —— 誤差是位置相依的梯度（Δcy 從 −0.011 線性增到 −0.062），
   單一 `cy_shift` 常數在數學上就修不了。cy 應擬合成 cx 的線性函數，多一個參數，幾張圖就夠。

改完再跑 `derive_vision_calibration.py --reference-labels <handoff raw>`，
那才會得出「LLM 的框對人工真值到底多準」，以及「幾張手標夠用」。

## 20. 匹配修好了；傾斜項不該加（2026-09-17）

§19 結尾列的兩件事，第一件做了，**第二件經量測否決**。

### 匹配：以 cx 為準

`match_by_centre` 改成 **cx 決定匹配、cy 只在 cx 完全同分時當 tiebreak**。
新增 `vertical_radius` 參數，讓 cy 只能當**閘門**、永遠不能當排序項；
預設不啟用，Cable1/A 的物件是單排、cx 相距 0.045，本來就不需要它。

**真實數字**（舊：`runs/vision_calibration_truth/` 09-15；新：`..._v2/` 09-17 真跑）：
舊匹配器 **146/275（53.1%）** → 新匹配器 **219/275（79.6%）**。

> 本節初稿的兩個數字都來自我的模擬提案者（cx 噪聲 ±0.004、cy 寫死 0.62），**都已作廢**：
> 「18/275」高估了舊匹配器的失敗，「275/275」高估了新匹配器。
> **模擬的參數是照「記錄下來的模型行為」設的，而那份記錄只有 6 張圖。**
> §19 記的「0/6」是某一張圖，不是整場。

### 舊匹配器讓 `cy_shift` 變成無法量測（這是比匹配率更重要的事）

二維閘門 `hypot(dx,dy) <= 0.02` 只收 **cy 已經對的框**。於是那次跑出來的
`cy_shift = +0.0091` 看起來像「cy 幾乎沒偏」——但它是先把所有 cy 錯的框丟掉，
再宣布 cy 沒問題。**自我印證。** 而 IoU 只到 0.438/0.451、達到 0.5 的僅約 1/3
（fit 26/68、holdout 26/78），缺口就在被丟掉的那 129 個框裡。

改成 cx 匹配之後 275 個框全部進來，`cy_shift_spread` 與 `vertical_structure`
才會量到真的東西。

### 傾斜項：不加，而且理由是量出來的

先量人工標註自己的幾何（不涉及任何 VLM，純真值）：

| 量 | 結果 |
| --- | --- |
| **同一張圖內**，框對擬合直線的最大偏離 | 中位數 **0.0047**、p90 0.0070、最差 0.0106（框高 0.09） |
| **跨圖**，那條線的斜率 | 中位數 +0.0673、stdev **0.1166**、**46 張中 16 張變號** |
| **跨圖**，cy@0.5 | stdev 0.1010，範圍 0.309–0.760 |

**斜率的離散度是自己中位數的 1.73 倍，而且 35% 的畫面往反方向傾。**
那不是站別的 bias，是逐幀變數。而 `BoxCalibration` 用 `applies_to()` 綁死站別，
逐幀的量放進去在型別上就是錯的。所以**沒有加 `cy_tilt`**。

§19 寫的「Δcy 從 −0.011 線性增到 −0.062，單一常數修不了」前半對、後半的結論錯：
單一常數確實修不了，但補救不是站別級的第二個常數。

### 真正的好消息：垂直只剩「每幀一個數」

同一張圖內共線到 0.0047，代表**一幀的垂直資訊是 1–2 個未知數，不是 6 個**。
扣掉每幀自己的中位數之後，幀內殘差 MAD 就是 0.0047 —— 框高的 1/19。

所以冷啟動的垂直問題被壓縮成：**每張圖取得一個垂直偏移**。
來源有三個候選（影像本身的焊點列、VLM 自己那條平線的高度、少量人工種子），
**還沒有選**，因為下一節那個問題會決定選哪個。

### 新增的量測，以及它要回答的問題

`measure_vertical_structure()`（放在 `vision_evidence.py`，不是 script，
因為它是支撐 `BoxCalibration` 設計的量測，且需要進 mypy 與測試）
把垂直誤差拆成三塊：`between_frames` / `within_frames` / `frame_slopes`，
並附一句 `reading` 直接講結論。**pairs 必須保持依幀分組**——攤平之後
「站別級」與「逐幀級」在數學上無法區分，而那正是要問的問題。

`BoxCalibration` 另加 `cy_shift_spread`（dy 的 MAD）。模擬跑出來
`cy_shift -0.0275 / spread 0.0659`——**位移比自己的離散度還小**，
這個欄位存在就是為了讓這件事一眼看得到，而不是把它讀成跟旁邊的 scale 一樣硬。

### VLM 的 cy 有沒有跟著每幀走？**有**——用上界論證離線證完，不必開端點

§19 只確認「不跟傾斜走」，沒分離出「跟不跟高度走」。這題不需要端點：

1. 舊匹配器的閘門是 `hypot(dx,dy) <= 0.02`，所以**凡是匹配上的框，必然 `|dy| <= 0.02`**。
2. 那次真的匹配了 **146** 個框。
3. 若 VLM 的 cy 是**同一個全域常數 k**，則匹配數的上限就是
   「人工真值 cy 落在 k ± 0.02 內的框數」。掃遍所有 k，**最大值是 86/275（31.3%，k=0.5735）**。
4. **146 > 86，矛盾。** 故 VLM 的 cy 不可能是全域常數。

（第 3 步與「哪個提案框配到哪個目標」無關：只要所有提案 cy 都等於 k，
任何被配上的目標都滿足 `|cy_truth − k| <= 0.02`，上界成立。）

**所以垂直的「每幀偏移」大部分已經在 VLM 的回答裡了**，不必另外找來源。
剩下的是殘餘偏移與傾斜。

補一個定位數字：若每幀給一條**平的線但高度用該幀自己的中位數**（只有偏移、沒有傾斜），
**248/275（90.2%）** 的框落在 0.02 內。這是「偏移解決大部分、傾斜是尾巴」的量化。

端點跑仍然值得做，但性質變了：是**確認**，不是探索。跑
`derive_vision_calibration.py --reference-labels <handoff raw>`，看 `vertical_structure`
與重新量到的 `cy_shift_spread`。

### 環境閘門（沒開，也沒繞過）

`QWEN_URL` 未設、`PICTURE_TOOL_VISION_CALLS` 未設 → 端點這條路是關的。
`ANTHROPIC_API_KEY`/`ANTHROPIC_BASE_URL` 是通的，但那是付費路徑，**沒有碰**。

### 順手修掉一個會隨時鐘腐壞的測試（與上述無關的一批）

`tests/test_autotrain_service.py` 的 `_write_snapshot` 把紀錄寫死在日期目錄
`20260910`，而 collector 的 `lookback_days` 預設 7。視窗是「今天往回 6 天」，
所以這兩個測試 **2026-09-16 還會過、09-17 開始失敗**，repo 一行都沒改。
改成以 `datetime.now()` 產生日期目錄與 timestamp。

已在 HEAD（`4ce9c13`）的乾淨 worktree 上確認這兩個失敗**先於本次改動**存在。

### 驗證

`mypy src/picture_tool` Success（148 檔）、ruff 乾淨、
**1575 passed / 6 skipped**（修測試前是 1573 passed / 2 failed）。

## 21. 真跑確認（2026-09-17，Qwen 端點）

端點位址在 workspace 根目錄的 `qwen.env`（**未被 git 追蹤，但也沒被 ignore ——
一次 `git add -A` 就會把內網位址提交進去**，待決定）。46 張人工真值全跑，
輸出在 `runs/vision_calibration_truth_v2/`，**沒有覆蓋** 09-15 那份。

### 垂直：§20 的結論全部成立，而且更強

| | 值 | 換算成框高（0.108） |
| --- | --- | --- |
| `between_frames` MAD | **0.0185**（範圍 −0.2631..+0.1237，14/46 負） | 0.17 |
| **`within_frames` MAD** | **0.0037**（n=219） | **0.03** |
| `frame_slopes` MAD | 0.0737（中位數 +0.0297，**16/41 變號**） | —— |

1. **沒有站別級 `cy_shift`**：`cy_shift +0.0106 / spread 0.0207`，**離散是位移的兩倍**。
2. **沒有站別級傾斜**：16/41 變號、MAD 是中位數的 2.5 倍。§20 的否決成立。
3. **垂直幾乎全是「每幀一個偏移」**：扣掉之後幀內殘差只剩 **0.03 個框高**。
   離線從純人工標註算到的是 0.0047，真跑是 0.0037，兩條獨立路徑對上。
4. **VLM 的 cy 確實跟著每幀走**：若是全域常數，`between_frames` 的 MAD 會落在
   0.10 量級（幀高 stdev），實測 0.0185。**§20 的上界論證得到獨立驗證。**

### 沒預料到的：現在的瓶頸是 cx，不是 cy

匹配率只有 79.6%，缺的 56 個框是**沒通過 cx 閘門**（cy 已經不設閘）。
所以**至少 20% 的框 `|dx| > 0.02`**。而 0.02 是**半個框寬**（框寬中位數 0.0406）。

§19 記的「cx ±0.005」是從 **6 張圖**量的，**不能外推到 46 張**。

**缺口（我自己造成的）**：`BoxCalibration` 只加了 `cy_shift_spread`，沒加
`cx_shift_spread`。當初的前提是「cx 可信、cy 不可信」，這次資料說 cx 也要同樣的
檢視。下次改：補 `cx_shift_spread`，並把匹配失敗的框也記下來
（現在它們靜默消失，匹配率是唯一線索）。

### IoU 下降了，但那不是退步

| | 舊（2-D 匹配，146 框） | 新（cx 匹配，219 框） |
| --- | --- | --- |
| fit  raw → cal | 0.355 → 0.438 | 0.288 → 0.398 |
| holdout raw → cal | 0.383 → 0.451 | 0.328 → 0.381 |
| usable (IoU≥0.5) | 26/68、26/78（≈35%） | 37/104、39/115（≈35%） |

**舊的平均值是在一個被挑過的子集上算的**——2-D 閘門只收 cy 已經對的框，
那些框本來就比較容易得到高 IoU。新匹配器多收 73 個較難的框，平均自然往下。
**達標率（≈35%）幾乎沒變，達標的絕對數量從 52 升到 76。**

換句話說：舊數字比較好看，是因為它量的是比較容易的題目。

### 下一步（依價值）

1. **補 `cx_shift_spread` 與未匹配框的記錄**，重跑一次——cx 現在是主要誤差來源，
   而我們對它的離散度一無所知。
2. 每幀偏移的來源：資料說它**已經在 VLM 的回答裡**（between MAD 0.0185），
   所以不需要另建焊點列偵測。真正要處理的是那幾個離群幀（min −0.2631）。
3. IoU 0.38 對訓練夠不夠，要用「訓出來的模型」回答，不是用 IoU 門檻猜。

## 22. §19–§21 的定位結論全部作廢：問題是 max_side=640（2026-09-18）

`VisionLLMEvidence` 預設 `max_side=640`。原圖 3072×2048、線頭約 125px，
縮完只剩約 26px。**§19 到 §21 所有關於「模型定位能力」的量測都在這個條件下做的。**

10 張分層對照（同樣影像、同樣提示，只改 max_side）：

| | 640 | **3072** |
| --- | --- | --- |
| 定位成功 | 59/60 | **60/60** |
| **cy 偏移中位數** | **0.0771** | **0.0065** |
| mean IoU | 0.226 | 0.319（加像素修正高度 **0.420**） |
| 每張耗時 | 26.7s | 31.0s |

最戲劇性的一張：640 下六個框 cy 全部吐出死平的 `0.41`（真值 0.68），IoU 0.000；
3072 下 cy 是 `0.673/0.670/0.670/0.664/0.670/0.671`，緊貼真值那條線。**偏移改善 20 倍。**

**所以「VLM 把 cy 壓平成常數、不從影像推導絕對垂直位置」是降採樣的產物，不是模型能力。**
§19 的那張表、§21 的 `between_frames`、以及「垂直是每幀一個偏移」的整套推論，
前提都是錯的。§20 的匹配修正本身仍然成立（cx 決定匹配是對的），
但它解決的問題規模被這件事改變了。

### 沒有全面變好，尺度偏差反而放大

好的影格變差（0.607→0.135、0.494→0.349）。cy 偏移正常，是尺寸出問題：
...
**看得越清楚，框畫得越小** —— 它看得到線頭本體就只框那一小段。
所以 3072 的尺度修正是 寬 ×1.324 / 高 ×1.981，**舊校正（×1.497）套過去會錯**。

### 還不能宣布成功

n=10；`temperature`/`seed` 未設，同輸入兩次跑定位差 12 個百分點；
3072 下仍有 0.135、0.201 的影格。46 張全量重跑**沒有做**。

### 這次最該記住的

我用模擬估真實行為錯了三次，又拿 640 的結果推論模型能力一次。
**共同點：在沒有確認量測條件是否公平之前就開始推論。**
下次面對「模型做不到 X」的結論，先問「我給它的輸入，人自己看得出 X 嗎？」
