...
class DataAugmentor:
    ...
    def _process_single_image(self, img_file: str) -> None:
        try:
            ...
            self.logger.info(f"處理 {img_file}, 原始尺寸: {original_width}x{original_height}")
            try:
            ...
            for i in range(self.config['augmentation']['num_images']):
                try:
                    ...
                    for bbox, label in zip(final_bboxes, augmented_labels):
                        if (bbox[2] > 0.001 and bbox[3] > 0.001 and
                            0 <= bbox[0] <= 1 and 0 <= bbox[1] <= 1):
                            valid_bboxes.append(bbox)
                            valid_labels.append(label)
                    if len(valid_bboxes) == 0:
                        self.logger.warning(f"沒有有效的最終標註，跳過 {img_file}_aug_{i+1}")
                        continue
                    if self.config['processing'].get('debug_mode', False):
                        debug_dir = Path(self.config['output'].get('debug_dir', 'debug_visualizations'))
                    ...
                    self.logger.info(f"輸出: {aug_img_filename} (含 {len(valid_bboxes)} 個框)")
                except Exception as e:
    ...
    def process_dataset(self):
        ...
        if self.config['processing'].get('debug_mode', False):
            ...
            self.logger.info("請檢查 debug_visualizations 目錄中的結果，確認無誤再全量處理")
            return
