...
def run_batch_inference(config: dict, logger: Optional[logging.Logger] = None) -> Path:
    ...
    device = str(icfg.get('device', config.get('yolo_training', {}).get('device', 'cpu')))
    ...
    logger.info(f"Batch infer using weights={weights_path} imgsz={imgsz} conf={conf} device={device} input={input_dir}")
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['file', 'class_id', 'class_name', 'conf', 'x1', 'y1', 'x2', 'y2'])
        for img_path in images:
            ...
            for res in results:
                ...
                logger.info(f"{img_path.name}: {len(confs)} detections (min_conf={min(confs) if confs else 'NA'})")
                for (x1, y1, x2, y2), c, cid in zip(xyxy, confs, clss):
                    cid_int = int(cid)
                    cname = names.get(cid_int, str(cid_int)) if isinstance(names, dict) else str(cid_int)
                    writer.writerow([img_path.name, cid_int, cname, float(c), float(x1), float(y1), float(x2), float(y2)])
