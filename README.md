...
- **新手導覽與流程範例**：首次啟動顯示簡易教學或提供 sample dataset，讓使用者可以照指引完成一次完整流程。
- **流程視覺化與進度追蹤**：在 GUI 左側加入任務流程圖或進度條，顯示各任務的依賴與目前狀態。
- **啟動前自動檢查**：新增「啟動前檢查」按鈕，驗證資料夾/權重是否存在、磁碟空間等，提前提示修正建議。
- **指標儀表板**：右側加入訓練指標（loss、mAP）、LED QC 信心分析等簡易圖表，減少翻查報表的步驟。
- **流程模板分享**：支援匯出/匯入 `preset_config.yaml`，或內建常用模板列表，方便跨專案複製流程。
- **通知與記錄整合**：完成流程後可選擇寄送 E-mail/Slack 通知，並集中 log/報表連結。
- **CLI Pipeline 入口**：針對整體流程提供單一命令列 entry point，讓 CI 或自動化腳本也能直接呼叫。
